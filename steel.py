...
img_size = 256
#导入数据集
tr = pd.read_csv('severstal-steel-defect-detection/train.csv')
print(len(tr))
print(tr.head())
#数据清洗（除去空缺值）
df_train = tr[tr['EncodedPixels'].notnull()].reset_index(drop=True)
df_train = df_train[df_train['ClassId'].apply(lambda x: x == 4)].reset_index(drop=True)
print(len(df_train))
print(df_train.head())
...
model.compile(tensorflow.keras.optimizers.Adam(lr=0.0001), 'binary_crossentropy')
batch_size = 8
...
# testfiles=os.listdir("severstal-steel-defect-detection/test_images/")
# len(testfiles)
# test_img = []
# for fn in tqdm_notebook(testfiles):
#         img = cv2.imread('severstal-steel-defect-detection/test_images/'+fn )
#         img = cv2.resize(img,(img_size,img_size))
#         test_img.append(img)
#
...
# img_t = cv2.imread( 'severstal-steel-defect-detection/test_images/'+ testfiles[4])
# plt.imshow(img_t)
# mask_t = rle2mask(pred_rle[4], img.shape)
# plt.imshow(mask_t)
# sub = pd.read_csv( 'severstal-steel-defect-detection/sample_submission.csv', converters={'EncodedPixels': lambda e: ' '} )
# sub.head()
